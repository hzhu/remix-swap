...
import { transformTokkenParamsToAddress } from "~/routes/swap/utils";
...
export function useFetchDebouncePrice(
  ...
  useEffect(() => {
    debouncedRef.current = debounce(async (params, network) => {
      const paramsWithAddy = transformTokkenParamsToAddress(params, network);
      const endpoint = ENDPOINTS[CHAIN_IDS[network]];
      const URL = `${endpoint}/swap/v1/price?${qs.stringify(paramsWithAddy)}`;
      try {
